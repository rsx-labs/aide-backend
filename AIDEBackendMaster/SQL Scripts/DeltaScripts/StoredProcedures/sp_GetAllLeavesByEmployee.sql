...
(
	SELECT ROW_NUMBER() OVER (ORDER BY DATE_ENTRY asc ) AS ROWNUMBER,R.DATE_ENTRY, R.COUNTS, R.STATUS_CD, R.STATUS, S.DESCR  FROM RESOURCE_PLANNER R INNER JOIN STATUS S ON 
		R.STATUS = S.STATUS WHERE S.STATUS_ID in (7,9) and R.EMP_ID = @EMP_ID and R.STATUS not in (7) and R.STATUS_CD = @STATUS_CODE and R.DATE_ENTRY >= CONVERT(DATE, GETDATE())
),
 CTEDATES1
AS
(
