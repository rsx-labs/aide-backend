...
GO
	...
	Select emp.EMP_ID as EmployeeID,
			...
			format(convert(date,task.DATE_CREATED,10),'MM/dd/yyyy') as DateCreated,
			format(convert(date,task.DATE_STARTED,10),'MM/dd/yyyyy') as DateStarted,
			format(convert(date,task.TARGET_DATE,10),'MM/dd/yyyy') as TargetDate,
			format(convert(date,task.COMPLTD_DATE,10),'MM/dd/yyyyy') as DateCompleted,
