...
IF EXISTS (SELECT * FROM sys.objects WHERE type = 'P' AND OBJECT_ID = OBJECT_ID('dbo.[sp_GetAllLeavesByEmployee]'))
    BEGIN
...
(
	...
	AND EMP_ID = @EMP_ID 
	AND R.STATUS NOT IN (7) 
	AND STATUS_CD = @STATUS_CODE 
	AND DATE_ENTRY >= GETDATE()
),
 CTEDATES1
AS
(
   ...
   SELECT a.ROWNUMBER, a.DATE_ENTRY, 
	...
	a.STATUS AS STATUS, 
	a.DESCR AS DESCR 
	FROM CTEDATES A INNER JOIN CTEDATES1 B ON A.ROWNUMBER-1 = B.ROWNUMBER
)
